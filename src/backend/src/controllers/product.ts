...
export const newProduct = TryCatch(
  async (req: Request<{}, {}, NewProductRequestBody>, res, next) => {
    ...
    if(photos.length < 1) return next(new ErrorHandler("Please add one Photo atleast", 400));

    if(photos.length > 5) return next(new ErrorHandler("Please add 5 Photo at max", 400));
    
    if (!name || !price || !stock || !category) {
      return next(new ErrorHandler("Please enter All Fields", 400));
    }

    // Upoload Image on Cloudinary
